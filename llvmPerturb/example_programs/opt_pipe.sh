...
if [[ $1 == "small" ]] || [[ $2 == "small" ]]; then
  ...
  opt -load "$HOME/llvm-8.0.0.src/build/lib/LLVMRandom.so" -Random -pp 2 < sump.bc > sump_opt.bc
...
if [[ "$1" == "count" ]] || [[ "$2" == "count" ]]; then
  ...
  opt -load "$HOME/llvm-8.0.0.src/build/lib/LLVMPerturbCount.so" -Count < linked_challenge_pone.bc > /dev/null
