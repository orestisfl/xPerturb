...
class TraceNsc2013():
    ...
    def accuireReferenceTrace(self):
        subprocess.Popen([self.BinaryPath + "src/nosuchcon_2013_whitebox_noenc_generator"], shell=True)
    ...
    def accuireTransformedTrace(self, prob, point):
        ...
        T=TracerGrind(self.BinaryPath + 'perturbations/nosuchcon_2013_whitebox_noenc', self.processinput, self.processoutput, ARCH.amd64, 16, addr_range='0x108000-0x130000') # filters=[DefaultFilters.stack_w1]
        T.run(self.runns) #25 Original
        bin2daredevil(config={'algorithm':'AES', 'position':'LUT/AES_AFTER_SBOX'}) # keyword=DefaultFilters.stack_w1,
